...
readme_pwd_path=$PWD/README.md

common_path=$PWD/src/common

echo "$(cat $common_path/README.md)" > $readme_pwd_path
echo '' >> $readme_pwd_path
echo "$(cat $common_path/README-containers.md | sed "s/\${NEOVIM_NAME}/[$(echo $NODES | sed -e "s/,/|/g")]/g")" >> $readme_pwd_path
echo "" >> $readme_pwd_path
echo "### Dockerhub links  " >> $readme_pwd_path
...
do  
    ...
    echo "$(cat $common_path/docker-compose.template.yml | sed "s/\${NEOVIM_NAME}/$node/g")" > $common_path/generated/docker-compose.$node.yml
